...
S2B Ormanization Spring Rest Docs API
...
[[users]]
== S2B organization User Apis

=== 사용자 생성 요청 (Request)
[[create-user]]
==== 사용자(Officer) 생성

`POST` 요청을 사용해서 새 사용자(Officer)를 만들 수 있다.
...
operation::query-users-pages[snippets='request-fields,curl-request,http-response,links']